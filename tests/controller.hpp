...
inline void independent_controller(
    std::function<void(const boost::json::object &, double &,
                       double &, double &)>
        _single_atom_lambda) {
  ...
  do {
    ...
    MPI_Recv(buffer, status._ucount, MPI_CHAR,
             status.MPI_SOURCE, status.MPI_TAG, comm, &status);
    buffer[status._ucount] = '\0';
    boost::json::object json =
        boost::json::parse(buffer).as_object();
    delete[] buffer;
    ...
    if (json["type"] == "register") {
      ...
      MPI_Send(raw.c_str(), raw.size(), MPI_CHAR,
               status.MPI_SOURCE, 0, comm);
    ...
    // Data processing
      ...
      MPI_Send(s.str().c_str(), raw.size(), MPI_CHAR,
               status.MPI_SOURCE, 0, comm);
    }
...
inline void dependent_controller(
    std::function<bool(const boost::json::array &)>
        _on_recv_all,
    std::function<void(const uint64_t &, double &, double &,
                       double &)>
        _single_atom) {
    ...
    // Data processing
      ...
      while (!_on_recv_all(list_to_send)) {
        for (const auto &p : bulk_received) {
          // Send waiting packet and continue
          const std::string msg = "{\"type\": \"waiting\"}";
          MPI_Send(msg.c_str(), msg.size(), MPI_CHAR, p.first,
                   0, comm);
        }
      }
...
inline void ffield_controller(
    std::function<void(const double[3], double[3])>
        _get_forces) {
  MPI_Comm comm, junk_comm;
    ...
    if (json["type"] == "register") {
      ...
      for (uint x_bin = 0; x_bin < node_counts[0]; ++x_bin) {
        for (uint y_bin = 0; y_bin < node_counts[1]; ++y_bin) {
          for (uint z_bin = 0; z_bin < node_counts[2];
               ++z_bin) {
            ...
            json_to_send.at("nodes").as_array().push_back(
                to_append);
          }
        }
      }
